...
function MenuPortal({ children }: { children: React.ReactNode }) {
  ...
  React.useLayoutEffect(() => {
    setTarget(
      (document.querySelector('.frontend-site') as HTMLElement | null) ?? document.body,
    );
  }, []);
          ...
          {mobileOpen ? (
            <>
              ...
              <motion.div
                ...
                className="fixed inset-y-0 right-0 z-90 flex w-[min(100%,22rem)] flex-col border-l border-fe-border bg-fe-surface shadow-2xl lg:hidden"
                role="dialog"
                aria-modal="true"
