...
interface MultiSelectorProps
  ...
  loop?: boolean;
  /** When set, selected chips show this label instead of the raw stored value (e.g. product name vs id). */
  badgeLabelByValue?: Record<string, string>;
}
...
>(({ className, children, ...props }, ref) => {
  const { value, onValueChange, activeIndex, badgeLabelByValue } =
    useMultiSelect();
      ...
      {value.map((item, index) => (
        ...
        >
          <span className="text-xs">
            {badgeLabelByValue?.[item] ?? item}
          </span>
          <button
            aria-label={`Remove ${badgeLabelByValue?.[item] ?? item} option`}
          ...
          >
            <span className="sr-only">
              Remove {badgeLabelByValue?.[item] ?? item} option
            </span>
