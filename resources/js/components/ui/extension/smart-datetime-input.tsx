...
// Validates the transformed date string for the natural language input (MMM D, YYYY[,] h:mm AM/PM)
  ...
  const handleParse = React.useCallback(
    (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (parsedDateTime) {
        ...
        const formatted = formatDateTime(parsedDateTime);
        if (!naturalInputValidationPattern.test(formatted)) return;
      ...
      switch (e.key) {
        case "Enter": {
          ...
          if (parsedDateTime) {
            ...
            const formatted = formatDateTime(parsedDateTime);
            if (!naturalInputValidationPattern.test(formatted)) break;
...
type DateTimeLocalInputProps = { disabled?: boolean | ((date: Date) => boolean) } & React.ComponentProps<typeof Calendar>;
